...
import PhysicsEngine.Objects.Native.*;
import PhysicsEngine.Objects.Parts.Ball;
import PhysicsEngine.Objects.Terain.SimplexNoise;
import PhysicsEngine.Objects.Terain.Terrain;
...
public class WorldData implements World, Physics {
    ...
    public void step(boolean useBallBallCollision) {
        ...
        if (useBallBallCollision) {
            ...
            for (int i = 0; i < amountOfThreads; i++) {
                RT[i] = new WorkThreadWithoutBallBallCollision(balls.size() * i / amountOfThreads, balls.size() * (i + 1) / amountOfThreads);
                RT[i].start();
            }
            for (int i = 0; i < amountOfThreads; i++) {
                try {
    ...
    @Override
    public void stepSimulated(ArrayList<Ball> simBalls, boolean useBallBallCollision, boolean noise) {
        boolean n = useNoise;
        useNoise = n;
    ...
    @Override
    public int getAmountWorldObjects() {
        return objects.size();
    }
    ...
    public boolean checkBallInHole(int i) {
        return hole.distance(balls.get(i).place) < (balls.get(i).size);
    }

    @Override
    public boolean ballStoppedMoving(int i) {
        return balls.get(i).zeroCounter >= Ball.thresholdCounter;
    }
    ...
    @Override
    public void load(String[][] data, double gs, Point3D offset) {
